...
initial=[0,1,1,1,0,0;1,0,0,0,1,1];
...
for iter=1:100
    ...
    zstr=[Hmin1;Hmin2]; zdst=[Hmin1;Hmin2];
    
    b=zeros(N,M,I); a=zeros(N,K,I);  t_pre=0.1.*ones(N,I); 
    Emax=[1500,1500];
...
%     zz=zz_data(:,:,iter);
    ...
    while(1)
        ...
        for i=1:I
            for n=1:N
                Wr(n,i)=sum(sum(a(1:n,:,i)*w)-sum(b(1:n,:,i)*v))+sum(initial(i,:)*v);
            end
        end
        ...
        for i=1:I
            for n=1:N
                W(n,i)=sum(sum(A(1:n,:,i)*w)-sum(B(1:n,:,i)*v))+sum(initial(i,:)*v);
            end
        end
    ...
    save("scenario1_user6","final_rate")
end
